...
class FileTreeSkillsAdapter(SkillsHarnessAdapter):
    ...
    def enable_shared_package(self, package_path: Path) -> None:
        ...
        dest = self.managed_root / package_path.name
        if dest.exists() or dest.is_symlink():
            raise MutationError(f"Directory or symlink already exists at {dest}; will not overwrite")
        self.managed_root.mkdir(parents=True, exist_ok=True)
        shutil.copytree(resolved_target, dest)

    def disable_shared_package(self, package_dir: str) -> None:
        dest = self.managed_root / package_dir
    ...
    def adopt_local_copy(self, existing_dir: Path, package_path: Path) -> None:
        ...
        shutil.copytree(resolved_target, existing_dir)
